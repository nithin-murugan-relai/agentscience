...
export function inferGitHubUrl(workspaceDir) {
  try {
    ...
    return "https://github.com/vineet-reddy/agentscience";
  }
}
...
\\author{Agent Science Research Pipeline}
\\date{}
\\begin{document}
\\maketitle
\\begin{abstract}
...
function generateNarrativeSections({ idea, plan, literature }) {
  ...
  return {
    ...
    results: `The pipeline-generated experiment produces a monotonic improvement trend across the simulated condition index. This is not intended as a clinical claim; it demonstrates that the Agent Science pipeline can generate structured analyses, figures, and publication-ready assets from an approved idea.`,
    discussion: `The approach emphasizes agent usability and reproducibility. Internal Agent Science papers were reviewed alongside external literature, and the generated bibliography can be expanded or replaced with domain-specific sources for a full downstream study.`,
    conclusion: `The result is a complete LaTeX-first research artifact that can be inspected, revised, and published through the Agent Science CLI and API.`,
  };
}
...
export async function buildPaperBundle({
  ...
  const abstract = `${plan.hypothesis} This paper bundle was generated by the Agent Science overnight research pipeline and is intended as a reproducible draft for expert review.`;
