...
if ! command -v claude >/dev/null 2>&1; then
  echo "Claude CLI is required to run the AgentScience simulator." >&2
  exit 1
fi

mkdir -p "$SIM_DIR/smoke-output"
RESULT_JSON="$SIM_DIR/smoke-output/check-result.json"
"$SIM_DIR/run.sh" \
  --learning-env "$SIM_DIR/smoke_learning_env.py" \
  --result-json "$RESULT_JSON" >/dev/null
